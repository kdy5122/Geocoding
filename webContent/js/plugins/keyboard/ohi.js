...
//mng_s 20221226 이진호
//영문서비스 일 땐 디폴트 언어가 영어가 되도록 수정
var default_ohi_KE = '';
if(document.location.href.match("english/map/interactiveMap")
		||document.location.href.match("thematicMap/categoryList_EN")){
	default_ohi_KE = 'En';
}else{
	default_ohi_KE = 'Ko';
}
...
function show_keyboard_layout(type) {
	//mng_s 20230413 이진호
	//초기 생성시 display none 처리
	$("body").append('<div id="vkey" style="display:none;"><div id="keyboardLayout"></div></div>');
	//mng_e 20230413 이진호
	...
	//mng_s 20221226 이진호
	//영문서비스 일 땐 KO/EN으로 보이도록 수정
	if(document.location.href.match("english/map/interactiveMap")
			||document.location.href.match("thematicMap/categoryList_EN")){
		ue.push(['Space','KO/EN','clear']);
	}else{
